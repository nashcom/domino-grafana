...
To start the Node Exporter just to to collect Domino statistics on Kubernetes (K8s) specify the following environment parameter for your Domino container.
...
Ensure port `9100` is exposed (Docker port configuration and Linux firewall).
  ...
  - job_name: domino
    static_configs:
      - targets:
        ...
        labels:
          pod: notes.example.com
          namespace: domino

      - targets:
          - domino.example.com:9100
        labels:
          pod: domino.example.com
          namespace: domino
```

The job should be called `domino` which turns into a label.
Targets could be configured in one step, but this does not allow to specify additional labels.

The target turns into the `instance` variable and isn't a stable variable over time.
On Kubernetes (K8s) the instance might be just the IP address of the pod.
K8s uses the `namespace` and `pod` label in dashboards.
...
## Run the Domino Compose stack

To start Grafana and Prometheus run the compose stack in detached mode.
...
In contrast to the existing [Domino Stats Publishing](https://help.hcltechsw.com/domino/14.0.0/admin/stats_publish_other_external.html)
functionality based on a **push model** (initially introduced for New Relic integration in Domino 10)
...
The **node_exporter** can be configured to send additional log files,
which is the best choice for Domino metrics collection on Linux to ensure metrics are in-line between the OS and Domino.
...
It is a single binary available on [GitHub](https://github.com/prometheus-community/windows_exporter/releases),
which should run on the host level to collect all relevant Windows statistics.

The **Windows Exporter** can be configured to send additional log files,
which is the best choice for Domino metrics collection on Windows to ensure metrics are in-line between the OS and Domino.


## Location of data

Data is stored in the following Docker volumes.
In contrast all configuration is stored in native volumes.

- **grafana-data**
- **prometheus-data**
- **loki-data** (currently unused)
